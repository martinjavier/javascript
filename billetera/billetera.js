function inicial()
{
    ...
    $('body').ready(() => {
        $.getJSON(misMonedasURL, function (respuesta, estado) {
                if(estado === "success"){
                var misMonedas = [];                     
                respuesta.forEach(e => {
                    var nombre = e.Nombre;
                    ...
                    misMonedas.push(new MiMoneda(nombre, precio, descripcion, cantidad));                       
                })
...
const editCoin = (id) => {
    ...
    console.log(JSON.stringify(todasMisMonedas[1].moneda).replace(/"/g,""));
    ...
    {        
        nombreMoneda = JSON.stringify(todasMisMonedas[i].moneda).replace(/"/g,"");
        if (nombreMoneda == id){
